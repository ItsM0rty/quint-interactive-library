...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
      return res.status(500).json({ error: 'GOOGLE_GENERATIVE_AI_API_KEY not set in .env file' });
    }
...
- You can also provide regular text responses without Quint blocks`;
    ...
    // Gemini supports full conversation history
...
app.listen(PORT, () => {
  ...
  if (!process.env.GOOGLE_GENERATIVE_AI_API_KEY) {
    console.warn(`WARNING: GOOGLE_GENERATIVE_AI_API_KEY not set in .env file`);
  } else {
    console.log(`Google API key loaded`);
    console.log(`Using model: Gemini 2.5 Flash (gemini-2.5-flash)`);
  }
});
